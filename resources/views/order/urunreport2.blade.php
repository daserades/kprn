...
@section('content')
	<div>
		<div>
			 <div align="center">{{ __('Ürün Bazında Satış Raporu Kod Bazında') }} 
			 	@isset($gtrh) -->({{$gtrh}}  @endisset @isset($ctrh) ---{{$ctrh}})  @endisset
			</div>
			<div class="col-md-12">
                        <form action="{{route('urunreportjs2')}}" method="post">
                        	@csrf
						...
						<tbody>
							...
							@foreach($urun as $list)
								<tr>
									...
									<td>{{$list->name ?? ''}} </td>
									<td>{{$list->mik ?? ''}} </td>
									<td> @if($list->urunkategori_id != 2)  {{$list->cmik ?? ''}} @else {{$list->mik ?? ''}} @endif </td>
									<td> </td>
								</tr>
							@endforeach
							@endisset
